...
router.route('/fetchReceipts').post(Auth.authenticateToken, async function (req, res) {
    try {
        ...
        const receipts = await History.find({ userId: req.body.userId, deleted: false, type: 'receipt', date: { $gt: moment(req.body.startDate).subtract('1', 'days'), $lte: moment(req.body.endDate).add('1', 'days') } }).sort({ date: -1 });
