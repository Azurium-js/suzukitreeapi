...
router.route('/sendInvoice').post(Auth.authenticateToken, async function (req, res) {
    try {
        ...
        const email = await Misc.sendEmail(
            ...
            "INVOICE",
            `<h2>
                <b>${student?.name !== "" ? student.name : "Anonymous"}'s Invoice</b>
            </h2>
            <br />
        ...
        return res.json({ message: "Successfully sent the tuition to the corresponding student's email address", status: "success" });
    } catch (err) {
