...
router.route('/fetchLessons').post(Auth.authenticateToken, async function (req, res) {
    try {
        ...
        const lessonTransactions = await History.find({ userId: req.body.userId, deleted: false, type: 'lessons', date: { $gt: moment(req.body.startDate).subtract('1', 'days'), $lt: moment(req.body.endDate).add('1', 'days') } }).sort({ date: -1 });
